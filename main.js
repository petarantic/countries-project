...
.then(response => {
  ...
  let ul =  document.getElementById('listOfCountries')
...
function clickFunction (li, i) { 
  li.onclick = function () {
    ...
    let fetchURL = 'https://restcountries-v1.p.rapidapi.com/name/';
...
function getQuote (fetchURL, name) {
  ...
  .then(response => response.json())
  .then(response => {
    let found = response.find(function (country, index) { 
      return country.name === name;
    });
    
    let capital = 'Capital: ' + found.capital;
    let population = 'Population: ' + found.population;
    let urlbeg= 'https://www.google.com/maps/embed/v1/place?key=AIzaSyAMUhZJImC93BePVKuTu17GN1En2mXhxS0&q=';
    let natName = 'Native name: ' + found.nativeName;
